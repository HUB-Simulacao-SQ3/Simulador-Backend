...
fs.readdir(path.join(__dirname + '/../../routes/urls'), async (err, files) => {
    if (err)
        throw new Error(err);
    const outputFile = path.join(__dirname + '/swagger_output.json');
    swaggerAutogen(outputFile, files.map((fileName) => path.join(__dirname + `/../../routes/urls/${fileName}`)), baseDocFile);
});
